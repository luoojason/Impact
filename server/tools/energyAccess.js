...
export default async function handler({ country }) {
  ...
  const url = `https://api.worldbank.org/v2/country/${iso2}/indicator/EG.ELC.ACCS.ZS?format=json&per_page=5`;
  ...
  return {
    ok: true,
    data: { date: latest.date, value: latest.value, indicator: 'EG.ELC.ACCS.ZS' },
    source: 'World Bank',
    url,
  };
}
